...
provides a source code generator that allows you to extend the core
cell interfaces, `ValueCell` and `MutableCell`, with accessors for
properties of your own classes.
...
In this example we used the `personForm` defined earlier. 
...
* The "Save" button saves the entered details, which held in the
  `person` cell.
