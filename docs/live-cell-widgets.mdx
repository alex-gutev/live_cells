...
collection of widgets which extend their stock Flutter widgets with
...
<CodeGroup title="CellText example">
```dart
...
In the example above a `CellText` with its data property bound to the
...
Widgets which are used to for retrieving input from the user, take
mutable cells for the properties which represents the user input. For
...
of the switch, similarly when the user changes changes the state of
the switch, the value of the cell is changed to reflect the state.

<CodeGroup title="CellSwitch example">
```dart
Widget example() => CellWidget.builder((c) {
...
`CellSwitch` doesn't take an `onChanged` callback.
...
<CodeGroup title="CellTextField example">
```dart
Widget example() => CellWidget.builder((c) {
	final content = c.cell(() => MutableCell(''));
...
<CodeGroup title="StaticWidget example">
```dart
...
<CodeGroup title="CellText example with StaticWidget">
```dart
Widget example() => StaticWidget.builder((c) {
...
<Info>
Like `CellWidget`, subclass `StaticWidget` and overriding the `build`
...
and your widget as a function:

<CodeGroup title="CellText example as a function">
```dart
Widget example() {
	final content = MutableCell('');
