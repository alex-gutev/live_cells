...
The simplest way to use the value of a cell is with
...
a good starting point, but it will quickly get annoying if you need to
...
directly in the widget builder function. The `cell` method, of the
`context` parameter passed to the widget builder function, creates a
cell which is persisted between builds of the widget.

This allows us to reimplement the counter example using the following:
...
In the example above the the `count` cell is defined directly in the
build function of `CellWidget.builder`, using `context.cell`.
...
This is functionally equivalent to the example using
...
In the example above, are two separate cells are defined in a single
...
`context.watch`. Like `context.cell`, the watch function is only setup
...
the widget in which it is defined is removed from the tree.
...
`CellInitializer` has to be included by the subclasses. This mixin
